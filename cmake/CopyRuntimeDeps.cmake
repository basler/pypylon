...
elseif(UNIX AND NOT APPLE AND (PYLON_FOUND OR pylon_FOUND))
    ...
    # Match the SDK root resolved in CMakeLists.txt (not only PYLON_ROOT env / /opt/pylon)
    if(DEFINED PYPYLON_PYLON_BASE_DIR AND PYPYLON_PYLON_BASE_DIR)
        set(_PYLON_SDK_ROOT "${PYPYLON_PYLON_BASE_DIR}")
    elseif(DEFINED ENV{PYLON_ROOT} AND NOT "$ENV{PYLON_ROOT}" STREQUAL "")
        set(_PYLON_SDK_ROOT "$ENV{PYLON_ROOT}")
    else()
        set(_PYLON_SDK_ROOT "/opt/pylon")
    endif()
    message(STATUS "Pylon SDK root for runtime copy: ${_PYLON_SDK_ROOT}")

    set(PYLON_LIB_DIR "${_PYLON_SDK_ROOT}/lib")
    ...
    if(RUNTIME_PYLON_VERSION VERSION_LESS "9.0.3")
        # Legacy naming scheme (Pylon < 9.0.3)
        set(BASE_PATTERNS
            "libpylonbase\\.so\\.[0-9]+$"
            "libGCBase_.*\\.so"
            "libGenApi_.*\\.so"
        ...
        # Current naming scheme (Pylon >= 9.0.3)
        set(BASE_PATTERNS
            "libpylonbase\\.so\\.[0-9]+$"
            "libpylonutility\\.so\\.[0-9]+$"
            "libGCBase_.*\\.so"
    ...
    if(PYLON_DATA_PROCESSING_FOUND)
        ...
        set(DP_PLUGIN_DIR "${_PYLON_SDK_ROOT}/lib/pylondataprocessingplugins")
        if(EXISTS "${DP_PLUGIN_DIR}")
            copy_directory_excluding_editors("${DP_PLUGIN_DIR}" "pylondataprocessingplugins")
        endif()
        
        set(DP_CREATOR_DIR "${_PYLON_SDK_ROOT}/lib/dataprocessingpluginsb")
        if(EXISTS "${DP_CREATOR_DIR}")
            copy_directory_excluding_editors("${DP_CREATOR_DIR}" "dataprocessingpluginsb")
        endif()
    endif()
...
elseif(APPLE AND (PYLON_FOUND OR pylon_FOUND))
    ...
    # Determine framework path (env, then CMake-detected base dir, then system default)
    if(DEFINED ENV{PYLON_FRAMEWORK_LOCATION} AND NOT "$ENV{PYLON_FRAMEWORK_LOCATION}" STREQUAL "")
        set(PYLON_FRAMEWORK_PATH "$ENV{PYLON_FRAMEWORK_LOCATION}")
    elseif(DEFINED PYPYLON_PYLON_BASE_DIR AND PYPYLON_PYLON_BASE_DIR)
        set(PYLON_FRAMEWORK_PATH "${PYPYLON_PYLON_BASE_DIR}")
    else()
        set(PYLON_FRAMEWORK_PATH "/Library/Frameworks")
    endif()
    message(STATUS "Pylon framework base for runtime copy: ${PYLON_FRAMEWORK_PATH}")
