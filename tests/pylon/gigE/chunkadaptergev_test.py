...
src/genicam/ChunkAdapterGEV.i.

A Basler GigE camera that supports Chunk mode is required.
...
class ChunkAdapterGEVTestSuite(PylonTestCase):
    ...
    def test_construction_no_args(self):
        """ChunkAdapterGEV() constructs without error."""
        adapter = genicam.ChunkAdapterGEV()
        self.assertIsNotNone(adapter)

    def test_construction_with_node_map(self):
        """ChunkAdapterGEV(nodeMap) constructs and attaches the node map in one step."""
        with pylon.InstantCamera(self.get_camera_traits(), pylon.FirstFound) as camera:
            adapter = genicam.ChunkAdapterGEV(camera.GetNodeMap())
            self.assertIsNotNone(adapter)
