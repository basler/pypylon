...
try:
    ...
    recipefilename = os.path.join(thisdir, 'dataprocessing_composite_data_types.precipe')
    ...
    imagespath = os.path.join(thisdir, 'images', 'shapes')
    ...
    for i in range(0, countOfImagesToGrab):
        if resultCollector.GetWaitObject().Wait(5000):
            ...
            # Print the found boxes
            variant = result["Boxes"]
            if not variant.HasError():
                # Print result data
                for barcodeIndex in range(0, variant.NumArrayValues):
                    print(variant.GetArrayValue(barcodeIndex).ToRectangleF())
            else:
                print("Error: " + variant.GetErrorDescription())
        else:
