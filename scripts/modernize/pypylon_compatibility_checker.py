    ...
    def compare_master_vs_local(self, output_format: str = "html", swig_version: str = "4.3", 
        ...
        try:
            ...
            if not master_dir.exists():
                ...
                # Apply patches to the cloned branch to make it build against new Pylon SDK
                print(f"🩹 Patching {branch} branch for compatibility with new Pylon SDK...")
                
                # Patch genicam.i
                genicam_i = master_dir / "src" / "genicam" / "genicam.i"
                if genicam_i.exists():
                    content = genicam_i.read_text()
                    if "#define GENICAM_INTERFACE struct" not in content:
                        content = content.replace("// definitions to directly include the SDK headers\n#define GENAPI_DECL", 
                                               "// definitions to directly include the SDK headers\n#define GENAPI_DECL\n#define GENICAM_INTERFACE struct")
                        genicam_i.write_text(content)
                
                # Patch pylon.i
                pylon_i = master_dir / "src" / "pylon" / "pylon.i"
                if pylon_i.exists():
                    content = pylon_i.read_text()
                    if "#define PYLON_INTERFACE struct" not in content:
                        # Add both PYLON_INTERFACE and GENICAM_INTERFACE
                        content = content.replace("#define interface struct\n#define PYLONUTILITY_API", 
                                               "#define interface struct\n#define GENICAM_INTERFACE struct\n#define PYLON_INTERFACE struct\n#define GENAPI_DECL_ABSTRACT\n#define PYLONUTILITY_API")
                        pylon_i.write_text(content)
            else:
                print(f"📂 Using existing checkout in {master_dir}")
